 ...
 * @version 2.0
 * @date 2024-03-28
 * @brief Peripheral Access Layer for S32K37_M7
 *
...
#if !defined(S32K37_COMMON_H_)  /* Check if memory map has not been already included */
#define S32K37_COMMON_H_
#define MCU_S32K37

/* Check if another memory map has not been also included */
#if (defined(MCU_ACTIVE))
  #error S32K37_M7 memory map: There is already included another memory map. Only one memory map can be included.
...
typedef enum {
  ...
  MCM_IRQn                     = 38,               /**< 'FPU OR'ed sources (Input Denormal, Inexact, Underflow, Overflow, Divide-by-Zero, Invalid Operand) ORed with MCM Write Abort interrupt */
...
#endif  /* #if !defined(S32K37_COMMON_H_) */
