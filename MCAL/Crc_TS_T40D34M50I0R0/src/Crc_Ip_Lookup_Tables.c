...
#define CRC_IP_LOOKUP_TABLES_SW_MAJOR_VERSION_C               5
                                   ...
                                   )
{
    ...
    switch (pxLogicChannelState->CrcProtocolInfo->Width)
    {
        case CRC_WIDTH_8BIT:
        #if (STD_ON == CRC_IP_8BIT_USED)
            LocResult = Crc_Ip_TableCalculateCRC8(pxLogicChannelState, LookUpTableAddress, DataPtr, Length, (uint8)InitialSeedStartValue);
        #endif
            break;

        case CRC_WIDTH_16BIT:
        #if (STD_ON == CRC_IP_16BIT_USED)
            LocResult = Crc_Ip_TableCalculateCRC16(pxLogicChannelState, LookUpTableAddress, DataPtr, Length, (uint16)InitialSeedStartValue);
        #endif
            break;

        case CRC_WIDTH_32BIT:
        #if (STD_ON == CRC_IP_32BIT_USED)
            LocResult = Crc_Ip_TableCalculateCRC32(pxLogicChannelState, LookUpTableAddress, DataPtr, Length, (uint32)InitialSeedStartValue);
        #endif
            break;

        case CRC_WIDTH_64BIT:
        #if (STD_ON == CRC_IP_64BIT_USED)
            LocResult = Crc_Ip_TableCalculateCRC64(pxLogicChannelState, LookUpTableAddress, DataPtr, Length, InitialSeedStartValue);
        #endif
            break;
