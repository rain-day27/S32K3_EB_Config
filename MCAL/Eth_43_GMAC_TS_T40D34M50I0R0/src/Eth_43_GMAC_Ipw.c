...
* @param[in]     CtrlIdx      Index of controller
* @param[in]     u8FifoIdx    The fifo index
* @param[in]     u32BufferIdx Current buffer index
...
uint32 Eth_43_GMAC_Ipw_GetNextBuffer(uint8 CtrlIdx, uint8 u8FifoIdx, uint32 u32BufferIdx)
{
    ...
    nextBufferIndex = (uint32)(u32BufferIdx + 1U);
    if (nextBufferIndex >= (Eth_43_GMAC_Ipw_axFirstTxBufferIdxInFifo[CtrlIdx][u8FifoIdx] + \
                            (uint32)((Eth_43_GMAC_Ipw_apxInternalCfg[CtrlIdx]->Eth_43_GMAC_Ipw_pGmacCtrlConfig->Gmac_paCtrlTxRingConfig)[u8FifoIdx].RingSize)))
    {
        nextBufferIndex = Eth_43_GMAC_Ipw_axFirstTxBufferIdxInFifo[CtrlIdx][u8FifoIdx];
    }
