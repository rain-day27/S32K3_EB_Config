...
#ifndef QSPI_IP_HWACCESS_H
#define QSPI_IP_HWACCESS_H
...
#define QSPI_IP_HW_ACCESS_SW_MAJOR_VERSION_H                5
...
{
#ifdef S32N2RT
    /*Todo: using SFP_TG_IPCR instead*/
#else
 ...
 * Enable DDR mode
 */
static inline void QSPI_DDR_Enable(const QuadSPI_Type *BaseAddr)
{
    /* Unused variable */
    (void)BaseAddr;
}

static inline void QSPI_DDR_Disable(const QuadSPI_Type *BaseAddr)
{
    /* Unused variable */
    (void)BaseAddr;
}
...
#else
static inline void Qspi_Ip_DLLEnableA(const QuadSPI_Type *BaseAddr,
                                      boolean Enable
                                     )
{
    /* Unused variable */
 ...
 */
static inline void Qspi_Ip_DLLSetDelayCoarseA(QuadSPI_Type *BaseAddr,
                                              uint8 CoarseDelay
                                             )
{
    /* get value DLLCRA register */
 ...
 */
static inline void Qspi_Ip_DLLSetDelayOffsetA(QuadSPI_Type *BaseAddr,
                                               uint8 CoarseDelay
                                              )
{
    /* get value DLLCRA register */
...
static inline boolean Qspi_Ip_DLLGetSlaveLockStatusA(const QuadSPI_Type *BaseAddr)
{
    ...
    /* get value DLLSR register */
    uint32 RegValue = (uint32)BaseAddr->DLLSR;
    /* set mask for LOCK */
    RegValue = (RegValue & QuadSPI_DLLSR_SLVA_LOCK_MASK);
...
static inline boolean Qspi_Ip_DLLGetLockStatusA(const QuadSPI_Type *BaseAddr)
{
    ...
    uint32 RegValue = (uint32)BaseAddr->DLLSR;
    /* set mask for LOCK */
    RegValue = (RegValue & QuadSPI_DLLSR_DLLA_LOCK_MASK);
    return (RegValue != 0U)? TRUE : FALSE;
#else
    (void)BaseAddr;
    /* For derivatives not support DLLSR register, the return value is always TRUE to avoid a timeout error */
...
#else
static inline void Qspi_Ip_SetDataInHoldTime(const QuadSPI_Type *BaseAddr,
                                             Qspi_Ip_FlashDataAlignType Enable
                                            )
{
    /* Unused variable */
...
{
    /* get value SR register */
    uint32 RegValue = (uint32)BaseAddr->SR;

    RegValue = (RegValue & QuadSPI_SR_BUSY_MASK);
    return (RegValue != 0U)? TRUE : FALSE;
}
...
static inline boolean Qspi_Ip_GetRxDataEvent(const QuadSPI_Type *BaseAddr)
{
    ...
    RegValue = (RegValue & QuadSPI_SR_RXWE_MASK);
    return (RegValue != 0U)? TRUE : FALSE;
}
...
static inline boolean Qspi_Ip_GetTxWatermarkAvailable(const QuadSPI_Type *BaseAddr)
{
    ...
    RegValue = (RegValue & QuadSPI_SR_TXWA_MASK);
    return (RegValue != 0U)? TRUE : FALSE;
}
#endif
