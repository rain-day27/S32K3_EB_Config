...
#define QSPI_IP_SW_MAJOR_VERSION_C             5
                                            ...
                                            )
{
    ...
    if (STATUS_QSPI_IP_SUCCESS == status)
    {
        ...
        else if (QSPI_IP_HYPER_RAM == state->configuration->memType)
        {
            status = Qspi_Ip_HyperRamInit(instance);
        }
        else
        {
            /*Do nothing*/
                                 ...
                                 )
{
    ...
    switch (state->configuration->memType)
    {
        ...
        case QSPI_IP_HYPER_RAM:
            status = Qspi_Ip_HyperRamReadId(instance,
                                            state->configuration->hrConfig->deviceIdWordAddress,
                                            data,
                                            state->configuration->readIdSettings.readIdSize);
            break;
 ...
 * Description   : Enables AHB reads for the current flash device
*/
Qspi_Ip_StatusType Qspi_Ip_AhbWriteEnable(uint32 instance)
{
#if (QSPI_IP_DEV_ERROR_DETECT == STD_ON)
    DEV_ASSERT_QSPI(instance < QSPI_IP_MEM_INSTANCE_COUNT);
#endif
    ...
    if (state->activeWriteLut != QSPI_IP_LUT_INVALID)
    {
        /* Copy sequence in LUT registers */
        (void)Qspi_Ip_InitLutSeq(instance, state->activeWriteLut, QSPI_IP_AHB_WRITE_LUT);

        /* Set sequence number */
        Qspi_Ip_SetAhbSeqWriteId(state->connection->qspiInstance, QSPI_IP_AHB_WRITE_LUT);
        status = STATUS_QSPI_IP_SUCCESS;
    }
                               ...
                               )
{
    ...
    state->activeWriteLut = pConfig->writeLut;
    state->lastCommand = QSPI_IP_LAST_COMMAND_NONE;
