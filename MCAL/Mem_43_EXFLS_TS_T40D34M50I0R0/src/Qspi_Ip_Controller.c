...
#define QSPI_IP_CONTROLLER_SW_MAJOR_VERSION_C                5
...
{
#ifdef QuadSPI_BFGENCR_SEQID_WR_MASK
    QuadSPI_Type *baseAddr;
    uint32 RegValue;
    ...
    RegValue = baseAddr->BFGENCR;

    /*Clear SeqId write bit field*/
    RegValue &= (~QuadSPI_BFGENCR_SEQID_WR_MASK);
    /*Set SeqId write and enable*/
    RegValue |= (QuadSPI_BFGENCR_SEQID_WR(seqID) | QuadSPI_BFGENCR_SEQID_WR_EN_MASK);
...
/*FUNCTION**********************************************************************
 *
 * Function Name : Qspi_Ip_IpWriteNormal
 * Description   : Launches an IP write command without sfp
 */
static Qspi_Ip_StatusType Qspi_Ip_IpWriteNormal(uint32 instance,
                                                uint8 SeqId,
                                                uint32 addr,
                                                const uint8 * data,
                                                uint32 size
                                  ...
                                  )
{
    ...
    #else /* FEATURE_QSPI_HAS_SFP */
        /* Launches an IP write command without sfp */
        status = Qspi_Ip_IpWriteNormal(instance, SeqId, addr, data, size);
    #endif /* FEATURE_QSPI_HAS_SFP */
