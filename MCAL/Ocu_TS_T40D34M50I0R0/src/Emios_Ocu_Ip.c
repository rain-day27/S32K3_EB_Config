...
#if (STD_ON == EMIOS_OCU_IP_DEV_ERROR_DETECT)
#include "Devassert.h"
#endif
...
#define EMIOS_OCU_IP_SW_MAJOR_VERSION_C                5
...
*/
static Emios_Ocu_Ip_GlobalDrvStateType Emios_Ocu_Ip_aesGlobalDrvState[OCU_EMIOS_INSTANCE_COUNT];
...
*/
static Emios_Ocu_Ip_ChannelStateType Emios_Ocu_Ip_saxChannelState[OCU_EMIOS_INSTANCE_COUNT][OCU_EMIOS_CHANNEL_COUNT];
...
static void Emios_Ocu_Ip_AssertChannelAndInstanceNumber(uint8 InstNum, uint8 ChNum)
{
    /* check that instance number and channel number are in valid range */
...
Emios_Ocu_Ip_ReturnType Emios_Ocu_Ip_SetRelativeThreshold(uint8 InstNum, uint8 ChNum, Emios_Ocu_Ip_ValueType RelativeValue)
{
    ...
    Emios_Ocu_Ip_ValueType CntValPostUpdate     = 0U;   /* CNT register value after computing the update */
    ...
    if (EMIOS_OCU_IP_DRV_STATE_IDLE == Emios_Ocu_Ip_aesGlobalDrvState[InstNum])
    {
        ...
        /* ensure the new compare match threshold does not exceed maximum counter value */
        while(CmpMatchValue > Emios_Ocu_Ip_sauMaxCounterVals[InstNum][ChNum])
        {
            CmpMatchValue = CmpMatchValue - (Emios_Ocu_Ip_sauMaxCounterVals[InstNum][ChNum] + 1U);
        }
 ...
 *          Note: the function applies the prescaler values only when the OCU channel is configured to use the internal counter (EMIOS_OCU_BUS_INTERNAL_COUNTER).
 *          In case the OCU channel is set to use a counter bus, the time base (including prescalers) is being given by the main EMIOS channel of that counter bus.
 *
 */
void Emios_Ocu_Ip_SetClockMode(uint8 InstNum, Emios_Ocu_Ip_SelectPrescalerType Prescaler)
{
    ...
    if (NULL_PTR != Emios_Ocu_Ip_aspxIpConfig[InstNum])
    {
        ...
        for (ChLoopIdx = 0U; ChLoopIdx < Emios_Ocu_Ip_aspxIpConfig[InstNum]->NumChannels; ++ChLoopIdx)
        {
            ...
            if((uint8)EMIOS_OCU_BUS_INTERNAL_COUNTER == ((pEmiosChannelCfg->ControlValue & EMIOS_OCU_BUS_SELECT_PARAM_MASK) >> EMIOS_OCU_BUS_SELECT_PARAM_SHIFT))
            {
                /* apply clock prescaler values from configuration only when EMIOS channel is using internal counter */
            ...
            {
                /* do not apply prescaler values since the selected channel is configured to use counter bus which means that the timebase (including prescale) is
                 * being given by the main channel of that counter bus */
            }
        }
...
#if (STD_ON == EMIOS_OCU_IP_DEV_ERROR_DETECT)
    Emios_Ocu_Ip_AssertChannelAndInstanceNumber(InstNum, ChNum);
#endif /* STD_ON == EMIOS_OCU_IP_DEV_ERROR_DETECT */
