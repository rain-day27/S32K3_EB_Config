...
#define OCU_IPW_SW_MAJOR_VERSION_C                5
...
void Ocu_Ipw_Init(const Ocu_Ipw_IpConfigType * const pIpConfig)
{
    ...
    if ((NULL_PTR != pIpConfig) && (NULL_PTR == Ocu_Ipw_pGlobalConfig[UserId]))
    {
        for(index = 0; index < OCU_EMIOS_INSTANCE_COUNT; index++)
        {
            ...
            {
                /* Call eMIOS IP init function for eMios instances configured configured on this partition */
                Ocu_Ipw_pGlobalConfig[UserId] = pIpConfig;
                Emios_Ocu_Ip_Init((*Ocu_Ipw_pGlobalConfig[UserId]->pEmiosIpConfig)[index]);
            }
        }
    }
}
...
void Ocu_Ipw_StartChannel(uint16 NumChannel)
{
    ...
    if (OCU_EMIOS_CHANNEL == (*Ocu_Ipw_pGlobalConfig[UserId]->pIpChannelsConfig)[NumChannel].mChannelIpType)
    {
        pEmiosIpChConfig = (*Ocu_Ipw_pGlobalConfig[UserId]->pEmiosIpConfig)[InstNum];
...
void Ocu_Ipw_SetClockMode(OCU_PIN_SELECT_PRESCALER_TYPE Prescaler)
{
    ...
    if ((NULL_PTR != Ocu_Ipw_pGlobalConfig[UserId]))
    {
        for(index = 0; index < OCU_EMIOS_INSTANCE_COUNT; index++)
        {
            if (NULL_PTR != (*Ocu_Ipw_pGlobalConfig[UserId]->pEmiosIpConfig)[index])
            {
