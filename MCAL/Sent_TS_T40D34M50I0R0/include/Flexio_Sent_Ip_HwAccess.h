...
#define FLEXIO_SENT_IP_HWACCESS_SW_MAJOR_VERSION               5
...
/*
 * This function configures the timer pin configuration.
 */
static inline void Flexio_Sent_Ip_SetTimerPinConfig(FLEXIO_Type *BaseAddr,
                                                    uint8 Timer,
                                                    Flexio_Mcl_Ip_PinConfigType PinConfig)
{
    uint32 Tmp = BaseAddr->TIMCTL[Timer];
    Tmp &= ~(FLEXIO_TIMCTL_PINCFG_MASK);
    Tmp |= FLEXIO_TIMCTL_PINCFG(PinConfig);
    BaseAddr->TIMCTL[Timer] = Tmp;
}
 ...
 */
static inline uint16 Flexio_Sent_Ip_GetTimerCompare(const FLEXIO_Type *BaseAddr,
                                                    uint8 Timer)
{
 ...
 * This function returns the compare value currently set for the specified timer.
 */
static inline void Flexio_Sent_Ip_SetTimerCompare(FLEXIO_Type *BaseAddr,
                                                  uint8 Timer,
                                                  uint16 CompareValue)
{
    BaseAddr->TIMCMP[Timer] = FLEXIO_TIMCMP_CMP(CompareValue);
}
 ...
 */
static inline void Flexio_Sent_Ip_SetTimerDMARequest(FLEXIO_Type *BaseAddr,
                                                     uint8 requestMask,
                                                     boolean enable)
{
    uint32 Tmp;
...
/*
 * This function configures corresponding pins to output logic one.
 */
static inline void Flexio_Sent_Ip_SetPinOut(FLEXIO_Type *BaseAddr,
                                            uint32 Pin)
{
    uint32 Tmp;
    Tmp = BaseAddr->PINOUTSET;
    Tmp |= FLEXIO_PINOUTSET_OUTSET((uint32)(1U) << (Pin));
    BaseAddr->PINOUTSET = Tmp;
}

/*
 * This function configures corresponding pins to output logic zero.
 */
static inline void Flexio_Sent_Ip_ClearPinOut(FLEXIO_Type *BaseAddr,
                                              uint32 Pin)
{
    uint32 Tmp;
    Tmp = BaseAddr->PINOUTCLR;
    Tmp |= FLEXIO_PINOUTCLR_OUTCLR((uint32)(1U) << (Pin));
    BaseAddr->PINOUTCLR = Tmp;
}

/*
 * This function configures corresponding pins to disable direct output.
 */
static inline void Flexio_Sent_Ip_DisablePinOut(FLEXIO_Type *BaseAddr,
                                                uint32 Pin)
{
    uint32 Tmp;
    Tmp = BaseAddr->PINOUTDIS;
    Tmp |= FLEXIO_PINOUTDIS_OUTDIS((uint32)(1U) << (Pin));
    BaseAddr->PINOUTDIS = Tmp;
}
